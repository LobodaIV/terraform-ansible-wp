...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile terransible && ansible-playbook -i aws_host wordpress.yml"
  }
}
...
resource "aws_elb" "wp_elb" {
  ...
  idle_timeout = 400 #recommended value
